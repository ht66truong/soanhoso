...
class DatabaseManager:
    ...
    def initialize_database(self):
        ...
        # Gọi hàm nhập mã ngành sau khi tạo bảng
        self.import_industry_codes_from_json()
        # Gọi hàm di chuyển dữ liệu nhân viên
        self.migrate_employees_from_json()

    def migrate_from_json(self, json_path):
        """Di chuyển dữ liệu từ file JSON sang cơ sở dữ liệu SQLite."""
        if not os.path.exists(json_path):
            logging.warning(f"File JSON không tồn tại: {json_path}")
            return False
        ...
        try:
            ...
            # Di chuyển các mục dữ liệu đã lưu
            current_config = data.get("current_config", "")
            entries = data.get("saved_entries", [])
            
            for entry in entries:
                entry_name = entry.get("name", "")
                entry_data = entry.get("data", {})
                
                # Loại bỏ thành viên và ngành nghề từ dữ liệu chính
                members = entry_data.pop("thanh_vien", []) if "thanh_vien" in entry_data else []
                industries = entry_data.pop("nganh_nghe", []) if "nganh_nghe" in entry_data else []
                
                # Thêm entry vào cơ sở dữ liệu
                cursor.execute(
                    """
                    INSERT OR REPLACE INTO entries (name, config_name, data, updated_at)
                    VALUES (?, ?, ?, datetime('now'))
                    """,
                    (entry_name, current_config, json.dumps(entry_data, ensure_ascii=False))
                )
                
                # Lấy ID của entry vừa thêm
                ...
                for position, industry in enumerate(industries):
                    is_main = 1 if industry.get("is_main", False) else 0
                    cursor.execute(
            ...
            logging.error(f"Lỗi khi di chuyển dữ liệu từ JSON: {str(e)}")
            return False
    ...
    # Hàm để nhập dữ liệu từ file JSON vào SQLite (chỉ cần chạy một lần)
    def import_industry_codes_from_json(self):
        """Nhập mã ngành từ JSON vào SQLite."""
        app_data_dir = os.path.dirname(self.db_path)  # Lấy thư mục AppData
        json_path = os.path.join(app_data_dir, "industry_codes.json")
        
        if not os.path.exists(json_path):
            logging.warning(f"File industry_codes.json không tồn tại: {json_path}")
            return False
        ...
        cursor.execute("SELECT COUNT(*) FROM industry_codes")
        count = cursor.fetchone()[0]
        
        if count == 0:  # Chỉ nhập nếu bảng trống
            try:
                with open(json_path, "r", encoding="utf-8") as f:
                ...
                for code in industry_codes:
                    cursor.execute(
                        "INSERT INTO industry_codes (ma_nganh, ten_nganh) VALUES (?, ?)",
                        (code["ma_nganh"], code["ten_nganh"])
                    )
                
                conn.commit()
                logging.info(f"Đã nhập {len(industry_codes)} mã ngành từ JSON vào SQLite")
            except Exception as e:
                logging.error(f"Lỗi khi nhập mã ngành: {str(e)}")
        
        conn.close()
        return True

    def migrate_employees_from_json(self):
        """Di chuyển dữ liệu nhân viên ủy quyền từ file JSON sang cơ sở dữ liệu SQLite."""
        json_path = os.path.join("AppData", "employees.json")
        if not os.path.exists(json_path):
            logging.warning(f"File employees.json không tồn tại: {json_path}")
            return False
            
        try:
            # Kiểm tra xem đã có dữ liệu trong bảng employees chưa
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM employees")
            count = cursor.fetchone()[0]
            
            # Chỉ nhập nếu bảng trống
            if count == 0:
                with open(json_path, 'r', encoding='utf-8') as f:
                ...
                conn.commit()
                logging.info(f"Đã nhập {len(employees)} nhân viên từ JSON vào SQLite")
            
            conn.close()
            return True
        except Exception as e:
            logging.error(f"Lỗi khi di chuyển dữ liệu nhân viên: {str(e)}")
            return False
